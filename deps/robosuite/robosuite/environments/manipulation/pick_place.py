...
class PickPlace(SingleArmEnv):
    ...
    def __init__(
        ...
        self.object_to_id = {"milk": 0, "bread": 1, "cereal": 2,
                             "can": 3, "bottle": 4, "lemon": 5, "cup": 6, "mug": 7}
        self.object_bin_assignment = {"milk": 0, "bread": 0, "cereal": 0,
                             "can": 1, "bottle": 3, "lemon": 0, "cup": 2, "mug": 2}
    ...
    def staged_rewards(self):
        ...
        if active_objs and r_grasp > 0.0:
            ...
            r_lift = grasp_mult + \
                (1 - np.tanh(15.0 * min(z_dists))) * (lift_mult - grasp_mult)

        # hover reward for getting object above bin
        r_hover = 0.0
        if active_objs:
            # target_bin_ids = [self.object_to_id[active_obj.name.lower()]
                              # for active_obj in active_objs]
            target_bin_ids = [self.object_bin_assignment[active_obj.name.lower()]
                              for active_obj in active_objs]
            ...
            y_check = (
                np.abs(
                    object_xy_locs[:, 1] - self.target_bin_placements[target_bin_ids, 1]) < self.bin_size[1] / 4.0
            )
            x_check = (
                np.abs(
                    object_xy_locs[:, 0] - self.target_bin_placements[target_bin_ids, 0]) < self.bin_size[0] / 4.0
            )
    ...
    def _create_obj_sensors(self, obj_name, modality="object"):
        ...
        def obj_to_eef_quat(obs_cache):
            return (
                obs_cache[f"{obj_name}_to_{pf}eef_quat"] if f"{obj_name}_to_{pf}eef_quat" in obs_cache else np.zeros(
                    4)
            )
